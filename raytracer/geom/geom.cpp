...
// basic geometry routines
	...
	struct shape { 
		...
		virtual bool calc_interception(const ray& ray, const nicefp& within_d, intercetion& out) = 0;
	...
	struct triangle: shape {
		...
		virtual bool calc_interception(const ray& ray, const nicefp &within_d, &interception out) override {
	...
	struct sphere: shape {
		...
		virtual sphere(vec3 acenter, vec3 orientationvec, vec3 orientation_equatorvec, nicefp amaxangle, nicefp aradius) { // maxangle in cos nicefps
		...
		virtual uv getuv(const vec3 &position) override { // fix this later!!!!
			...
			double u = atan2(orientation_equator.cross(equat_delta).norm() / orientation_equator.dot(equat_delta)) / (2 * M_PI); 
