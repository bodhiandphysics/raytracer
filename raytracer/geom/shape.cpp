...
namespace geom {
	...
	triangle::triangle(vec3 aposition, vec3 aside_a, vec3 aside_b, vec3 anormal, uv uv_a, uv uv_b, uv uv_c) {
	...
	void triangle::translate(const vec3 &by)  {

		position = position + by;
	}


	void triangle::rotate(const mat3 &by)  { // don't rotate triangles unless to absolutely must 

		position.rotate(by);
		side_a.rotate(by);
		side_b.rotate(by);

		normal.rotate(by); // update normal!!! (could rotate, but this prevents errors?)
	...
	bool triangle::calc_interception(ray& ray, nicefp within_d, interception& out)  {
		...
		if (determinant == 0) return false; // parallel to triangle
		...
		t = q.dot(edge2);
		if (t < 0 ) return false; // missed triangle
	...
	uv triangle::getuv(vec3 &intersect) { //using barycentric cooredinate
		...
		if (area == 0) return uv_a; // cut out early for really small triangles
	...
	sphere::sphere(vec3 acenter, vec3 orientationvec, vec3 orientation_equatorvec, nicefp amaxangle, nicefp aradius) { // maxangle in cos nicefps
	...
	vec3 sphere::get_normal(const vec3 &position)  {
		...
		return (center - position) / (radius * -1); // i hate hate hate that you cant declare const this!  

	}

	void sphere::translate(const vec3 &by)  {
	...
	void sphere::scale(const vec3 &by)  { // note elipses are not supported!!!!  make sure all scales are diagonal

		center = center.mult(by);
		radius = radius * by.x;  // acchch this is wrong!! 
	}

	bool sphere::calc_interception(ray& ray, const nicefp within_d, interception& out) {
		...
		nicefp descriminant = (d_dot_o_c * d_dot_o_c) - (d_dot_d *(delta_o_c.dot(delta_o_c) - radius*radius));
		...
		if (distance_m < distance_p && distance_m.value > 0 && distance_m < within_d) {
				...
				vec3 possible_position = delta_o_c + ray.direction * distance_m ; // i.e. orign + direction* distance - center
		...
		if (distance_p.value > 0 && distance_p < within_d) {
			...
			vec3 possible_position = delta_o_c + ray.direction * distance_p; // i.e. orign + direction* distance - center
	...
	uv sphere::getuv(vec3 &position)  {
		...
		double u = atan2(orientation_equator.cross(equat_delta).norm().value, orientation_equator.dot(equat_delta).value) / (2 * M_PI);
	...
	vec3 plane::get_normal(const vec3 &position)  {
