...
struct ray {
  ...
  bool refract(const vec3 &point, const vec3 &normal, const nicefp &inref_index,
               const nicefp &outref_index, ray &result);
};
...
struct shape {
  ...
  virtual bool calc_interception(const ray &theray, const nicefp within_d,
                                 interception &out);
...
struct triangle : shape {
  ...
  triangle(vec3 aposition, vec3 aside_a, vec3 aside_b, uv uv_a, uv uv_b,
           uv uv_c);
