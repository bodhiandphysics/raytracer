...
namespace camera {
	...
	struct camera {
		...
		camera(vec3 position, vec3 direction, vec3 image_udirection, vec3 image_vdirection, nicefp	focal_length, nicefp u_size, nicefp v_size, int height, int width);
		~camera();

		void render(const char* filename, world::world &theworld, int cutoff, double maxdistance);


	private:

		void redertask(int start, int stride, world::world &theworld, int cutoff, double maxdistance);
