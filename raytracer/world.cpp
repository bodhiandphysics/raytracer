...
void surfacegroup::rotate(mat3 &by) {
  ...
  for (surf::surface *asurface : surfaces)
    surface->shape.rotate(by);
}

void surfacegroup::translate(vec &by) {

  for (surf::surface *asurface : surfaces)
    surface->shape.translate(by);
}

void surfacegroup::scale(double factor) {

  for (surf::surface *asurface : surfaces)
    surface->shape.scale(by);
}
...
box::box() {
  ...
  surfaces.addshape<triangle>(triangle(vec3(0, 0, 0), vec3(0, 1, 0),
                                       vec3(1, 0, 0), vec3(0, 0, 1), uv(0, 0),
                                       uv(0, 0) uv(0, 0)));
  surfaces.addshape<triangle>(triangle(vec3(1, 1, 0), vec3(-1, 0, 0),
                                       vec3(0, -1, 0), vec3(0, 0, 1), uv(0, 0),
                                       uv(0, 0) uv(0, 0))); // + z triangles

  surfaces.addshape<triangle>(triangle(vec3(0, 0, -1), vec3(0, 1, 0),
                                       vec3(1, 0, 0), vec3(0, 0, -1), uv(0, 0),
                                       uv(0, 0) uv(0, 0)));
  surfaces.addshape<triangle>(triangle(vec3(1, 1, -1), vec3(-1, 0, 0),
                                       vec3(0, -1, 0), vec3(0, 0, -1), uv(0, 0),
                                       uv(0, 0) uv(0, 0))); // - z triangles

  surfaces.addshape<triangle>(triangle(vec3(0, 0, 0), vec3(0, 0, -1),
                                       vec3(0, 1, 0), vec3(-1, 0, 0), uv(0, 0),
                                       uv(0, 0) uv(0, 0)));
  surfaces.addshape<triangle>(triangle(vec3(0, 1, -1), vec3(0, 0, 1),
                                       vec3(0, -1, 0), vec3(-1, 0, 0), uv(0, 0),
                                       uv(0, 0) uv(0, 0))); // - x triangles

  surfaces.addshape<triangle>(triangle(vec3(1, 0, 0), vec3(0, 0, -1),
                                       vec3(0, 1, 0), vec3(1, 0, 0), uv(0, 0),
                                       uv(0, 0) uv(0, 0)));
  surfaces.addshape<triangle>(triangle(vec3(1, 1, 0), vec3(-1, 0, 0),
                                       vec3(0, -1, 0), vec3(1, 0, 0), uv(0, 0),
  ...
  surfaces.addshape<triangle>(triangle(vec3(0, 0, 0), vec3(0, 0, -1),
                                       ...
                                       uv(0, 0) uv(0, 0)));
  surfaces.addshape<triangle>(triangle(vec3(1, 0, -1), vec3(0, 0, 1),
                                       vec3(-1, 0, 0), vec3(0, -1, 0), uv(0, 0),
                                       uv(0, 0) uv(0, 0))); // - y triangles

  surfaces.addshape<triangle>(triangle(vec3(0, 1, 0), vec3(0, 0, -1),
                                       vec3(1, 0, 0), vec3(0, 1, 0), uv(0, 0),
                                       uv(0, 0) uv(0, 0)));
  surfaces.addshape<triangle>(triangle(vec3(1, 1, -1), vec3(0, 0, 1),
                                       vec3(-1, 0, 0), vec3(0, 1, 0), uv(0, 0),
                                       uv(0, 0) uv(0, 0))); // + y triangles
}
...
bal::ball(nicefp radius) {
  ...
  surfacegroup.addshape<geom::sphere>(
      sphere(vec3(0, 0, 0), vec3(0, 1, 0), nicefp(-1), radius));
}

world::world(camera::camera *thecamera, color lightcolor) {

  surfaces = std::vector<surf::surface *>() lights = std::vector<surf::light>();
...
world::~world() {
  ...
  for (surf::surace *surface : surfaces)
    delete surface;
  delete thecamera;
}
...
void world::addsurfacegroup(surfacegroup &group) {
  ...
  for (surf::surface *surface : group)
