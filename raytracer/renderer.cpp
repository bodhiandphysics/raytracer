...
namespace camera {
	...
	void camera::rendertask(int start, int stride, world::world theworld, int cutoff, double maxdistance) {
		...
		double u_scale = this.u_size / this.width;  // half sizes of image
		...
		for (int i = -width - 1 + start; i < width; i += stride) {
			...
			for (int j = -height - 1; j < height; j++) {
				...
				image[i + width + 1][j + height + 1] = raytrace(lightray, theworld, cutoff, nicefp(maxdistance));
			}
		}
	}

	void camera::render(const char* filename, world::world &theworld, int cutoff, double maxdistance) { // this is dumb multithreading, but it should work on average
