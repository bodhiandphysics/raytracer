...
surf::surface *find_next_surface(const rayoflight &theray,
  ...
  for (surf::surface &checksurf : theworld.surfaces) {
    ...
    didintercept =
        checksurf->shape.calc_interception(theray.ray, cutoff, intercept);
    if (didintercept) {
      retsuf = checksurface;
...
color cast_to_light(const ray &theray, const world::world &theworld,
                    cons const surface::light &thelight, nicefp starting_n,
                    nicefp maxdistance) {
  ...
  while ((surf::surface *next_surface = find_next_surface(
    ...
    if (theray.currentbeer.norm2() > nicefp(200))
...
color raytrace(ray &theray, world::world theworld, int cutoff,
  ...
  if (next_surface == nullptr)
    return theworld.getbgcolor(theray.theray); // we went off the map
  ...
  for (surface::light &thelight : theworld.lights) {
    ...
    nicefp currentn;
    if (direction_to_light.dot(next_surface.normal) < 0)
      current_n = next_material->refract_index; // going through new material
    else
      current_n =
    ...
    color shadowlightcolor =
        cast_to_light(shadowray, theworld, thelight, current_n, maxdistance);
    color += next_material
                 ->bdfrfactor(uv, shadowray.theray.direction,
                              theray.theray.direction, intercept.normal)
  ...
  if (next_material->doesfresnel) {
    double schlickt1 =
        pow(((next_material->refract_index - current_material->refract_index) /
            ...
            2);
    double costheta =
        reflectray.theray.direction.dot(next_material.normal).value;
    double schlick = schlickt1 + (1 - schlickt1) * pow((1 - costheta), 5);
    fresnelterm = nicefp(shlick);
  }

  color += (next_material->bdfrfactor(uv, shadowray.ray.direction,
                                      theray.ray.direction, intercept.normal) *
            fresnelterm)
               .mult(refelctlightcolor);

  // then handle refraction

  ray refractray;
  color refractlightcolor = vec3(0, 0, 0);
  bool didrefract = geom::refract(intercept.position, intercept.normal,
  ...
  if (theray.currentbeer == 0)
    return color / intercept.distance; // don't do nexp if not nescessary
  else
    return color.nexp(theray.currentbeer * intercept.distance) /
